# Authorship and Proof of Work

## Project

- Project: Squad Obliteration - CCL [DevKit]
- Author: **Enoc Bernal**

## Authorship Statement

This repository contains my Squad Obliteration mode implementation and related project artifacts prepared for open-source publication and portfolio review.

I authored the Squad Obliteration gameplay mode code and project-specific integration in this repository.

## Contribution Boundaries

- **Enoc Bernal**: Squad Obliteration mode implementation, gameplay logic, and project integration.
- **Michael De Luca**: Original template/tooling foundation used as the base environment.
- **Legacy logic inspirations/contributors**: BattlefieldDad, Mancour, uberdubersoldat, dfk_7677.

## Timeline Snapshot

- 2026-03-04: Open-source readiness pass completed (licensing, docs, attribution, provenance artifacts).

## Verifiable Artifacts

- `PROVENANCE.sha256` provides SHA256 hashes for key source/map files.
- Git commit history and release tags should be used as canonical publication timestamps.
